...
#include <stdint.h> // 定义标准整数类型，例如 int16_t, uint8_t
#include <stdlib.h> // 标准库函数，例如 abs() 取绝对值
...
float Kp = 35.0f; // 比例系数P：偏差越大，修正力度越大（主要控制参数，需要调试）
                  // 建议从20-50开始调试，太大会震荡，太小反应慢
float Ki = 0.0f;  // 积分系数I：消除长期累积偏差（循迹不需要，保持为0）
float Kd = 20.0f; // 微分系数D：根据偏差变化趋势进行预测，抑制震荡
                  // 可以让小车过弯更平滑，减少左右摇摆
...
int main(void) {
  ...
  // ========== 系统初始化 ==========
  Motor_Init(); // 初始化电机驱动：启动PWM通道，激活TB6612驱动芯片
  PID_Init();   // 初始化PID控制器：清零上一次偏差值
  ...
  while (1) // 主循环：程序会一直在这里运行
  {
    /* USER CODE END WHILE */

    /* USER CODE BEGIN 3 */
    // ========== 板载LED闪烁（用于指示程序正在运行）==========
    HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_13); // 翻转PC13引脚状态（LED亮/灭切换）
    HAL_Delay(1000);                        // 延时1000毫秒（1秒）

    // ========== 电机测试代码 ==========
    // 测试1：两个电机同时前进
    Set_Motor_Speed(500, 500); // 设置左右轮速度都为500（满速的50%）
    HAL_Delay(2000);           // 持续前进2秒

    // 测试2：停止
    Set_Motor_Speed(0, 0); // 设置左右轮速度都为0，电机停转
    HAL_Delay(1000);       // 停止1秒

    // 测试3：两个电机同时后退
    Set_Motor_Speed(-500, -500); // 负数表示反转，电机后退（速度为满速的50%）
    HAL_Delay(2000);             // 持续后退2秒
...
int16_t Calculate_Deviation(uint8_t sensor_value) {
  ...
  // **计算平均偏差**
  // 例如：如果bit0和bit1都检测到线（偏右）
  //       sum_weighted = 20 + 10 = 30
  //       sum_active = 2
  //       error = 30 / 2 = 15（正值，需要向左转）
...
void PID_Init(void) {
  ...
  // Kp, Ki, Kd 三个参数已经在文件开头的全局变量中初始化了
}
